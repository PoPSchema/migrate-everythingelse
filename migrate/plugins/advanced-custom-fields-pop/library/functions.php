...
function gdAcfUpdateValue($value, $post_id, $field)
{
    ...
    if (in_array($key, gdAcfGetKeysStoreAsArray())) {
        if (is_numeric($post_id)) {
            \PoP\PostMeta\Utils::updatePostMeta($post_id, $key, $value);
        ...
        if (is_numeric($post_id)) {
            \PoP\PostMeta\Utils::updatePostMeta($post_id, $key, $value, true);
        } elseif (strpos($post_id, 'user_') !== false) {
            $user_id = str_replace('user_', '', $post_id);
...
function gdAcfLoadValue($value, $post_id, $field, $keys, $single = false)
{
    ...
    if (in_array($key, $keys)) {
        ...
        if (is_numeric($post_id)) {
            return \PoP\PostMeta\Utils::getPostMeta($post_id, $key, $single);
        } elseif (strpos($post_id, 'user_') !== false) {
            $user_id = str_replace('user_', '', $post_id);
