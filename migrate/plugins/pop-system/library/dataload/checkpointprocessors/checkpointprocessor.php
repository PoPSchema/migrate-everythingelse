        ...
        switch ($checkpoint[1]) {
            case self::CHECKPOINT_SYSTEMACCESSKEYVALID:
                ...
                if (!POP_SYSTEM_APIKEYS_SYSTEMACCESS) {
                    return new \PoP\ComponentModel\Error('systemaccesskeynotdefined');
                }

                // Validate the user has provided the System Access Key as a param in the URL
                $key = $_REQUEST['systemaccesskey'];
                if (!$key) {
                    return new \PoP\ComponentModel\Error('systemaccesskeyempty');
                }

                // Validate the keys match
                if ($key != POP_SYSTEM_APIKEYS_SYSTEMACCESS) {
                    return new \PoP\ComponentModel\Error('systemaccesskeyincorrect');
                }
                break;
            ...
            case self::CHECKPOINT_SYSTEMACCESSIPVALID:
                ...
                if (!POP_SYSTEM_IPS_SYSTEMACCESS) {
                    return new \PoP\ComponentModel\Error('systemaccessipsnotdefined');
                }

                // Validate the user's IP
                $ip = getClientIp();
                if (!$ip) {
                    return new \PoP\ComponentModel\Error('systemaccessipempty');
                }

                // Validate the keys match
                if (!in_array($ip, POP_SYSTEM_IPS_SYSTEMACCESS)) {
                    return new \PoP\ComponentModel\Error('systemaccessipincorrect');
                }
                break;
        }
